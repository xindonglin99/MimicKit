...
class TaskSteeringEnv(amp_env.AMPEnv):
    def __init__(self, config, num_envs, device, visualize):
        ...
        self._enable_rand_tar_dir = env_config.get("enable_rand_tar_dir", True)
        ...
        self._reward_steering_dir_w = float(env_config["reward_steering_dir_w"])
        self._reward_steering_facing_w = float(env_config["reward_steering_facing_w"])
    ...
    def _build_sim_tensors(self, config):
        ...
        self._tar_facing_dir = torch.zeros([num_envs, 2], device=self._device, dtype=torch.float)
    ...
    def _get_facing_marker_id(self):
        return self._facing_marker_ids[0]
    ...
    def _update_marker(self, env_ids):
        ...
        facing_marker_pos[..., 0:2] += tar_dist_min * tar_facing_dir
        facing_marker_pos[..., 2] = 0.0

        facing_theta = torch.atan2(tar_facing_dir[..., 1], tar_facing_dir[..., 0])
        facing_axis = torch.zeros_like(root_pos)
        facing_axis[..., -1] = 1.0
        facing_marker_rot = torch_util.axis_angle_to_quat(tar_axis, facing_theta)


        self._engine.set_root_pos(env_ids, marker_id, marker_pos)
        self._engine.set_root_rot(env_ids, marker_id, marker_rot)
        self._engine.set_root_vel(env_ids, marker_id, 0.0)
        self._engine.set_root_ang_vel(env_ids, marker_id, 0.0)

        self._engine.set_root_pos(env_ids, facing_marker_id, facing_marker_pos)
        self._engine.set_root_rot(env_ids, facing_marker_id, facing_marker_rot)
        self._engine.set_root_vel(env_ids, facing_marker_id, 0.0)
        self._engine.set_root_ang_vel(env_ids, facing_marker_id, 0.0)
    ...
    def _reset_task(self, env_ids):
        ...
        if (self._enable_rand_tar_dir):
            ...
            rand_face_theta = torch.zeros(n, device=self._device)
    ...
    def _compute_obs(self, env_ids=None):
        ...
        if (env_ids is not None):
            ...
            tar_face_dir = tar_face_dir[env_ids]
        
        task_obs = compute_steering_observations(root_rot, tar_dir, tar_speed, tar_face_dir)
    ...
    def _update_reward(self):
        ...
        self._reward_buf[:] = compute_steering_reward(root_pos=char_root_pos,
                                                      ...
                                                      dir_reward_w=self._reward_steering_dir_w,
                                                      facing_reward_w=self._reward_steering_facing_w)
        return
    ...
    def _ig_build_env(self, env_id, config):
        ...
        if (self._visualize):
            marker_id, facing_marker_id = self._ig_build_markers(env_id)

            if (env_id == 0):
                self._marker_ids.append(marker_id)
                self._facing_marker_ids.append(facing_marker_id)
            else:
                marker_id0 = self._marker_ids[0]
                facing_marker_id0 = self._facing_marker_ids[0]

                assert(marker_id0 == marker_id)
                assert(facing_marker_id0 == facing_marker_id)
    ...
    def _ig_build_markers(self, env_id):
        ...
        facing_marker_id = self._engine.create_actor(env_id=env_id, 
                                             asset=self._marker_asset, 
                                             name="facing_marker", 
                                             col_group=col_group, 
                                             col_filter=col_filter, 
        ...
        return marker_id, facing_marker_id
...
@torch.jit.script
def compute_steering_observations(root_rot, tar_dir, tar_speed, tar_face_dir):
    ...
    local_tar_face_dir = torch_util.quat_rotate(heading_rot, tar_face_dir3d)
...
#@torch.jit.script
def compute_steering_reward(root_pos, prev_root_pos, root_rot, tar_dir, tar_speed, tar_face_dir, dt,
                           vel_err_scale, tangent_err_scale, dir_reward_w, facing_reward_w):
    ...
    dir_reward = torch.exp(-vel_err_scale * tar_vel_err - tangent_err_scale * tangent_vel_err)
